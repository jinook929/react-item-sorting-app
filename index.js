...
class ProductTable extends React.Component {
  render() {
    ...
    const inStockProducts = filterTextProducts.filter(product => product.stocked);

    const categorizedProductList = (targetProducts) => {
      const categories = targetProducts.map(product => product.category);
      const categoryArrays =[];
      targetProducts.forEach((product, i) => {
        let categoryId = categories.indexOf(product.category);
        // When a new category, create an array for the category with a header row
        ...
        // Add the item to the array of its category
        categoryArrays[categoryId].push(
          <ProductRow
          product={product}
          key={product.name} />
    ...
    if(inStockOnly) { // for inStockOnly products
      categorizedProductList(inStockProducts);
    } else {                  // for all products
      categorizedProductList(filterTextProducts);
    }
